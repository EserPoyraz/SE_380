...
class _ProgramBuilderPageState extends State<ProgramBuilderPage>
  ...
  Future<void> _generateWithAiAndSave() async {
    ...
    try {
      ...
      _snack('Zaman aşımı. Tekrar dene.');
    } catch (e) {
      _snack('Program üretilemedi.');
    } finally {
  ...
  Widget _buildCreateTab() {
    return ListView(
      ...
      children: [
        ...
        _choiceCard('Lokasyon', ['Home', 'Gym'], _location,
            (v) => setState(() => _location = v)),
        const SizedBox(height: 12),
        _choiceCard('Split', ['Push/Pull/Legs', 'Upper/Lower'], _split,
            (v) => setState(() => _split = v)),
        const SizedBox(height: 12),
        _choiceCard('Hedef', ['Strength', 'Fat Loss', 'Conditioning'], _goal,
            (v) => setState(() => _goal = v)),
        const SizedBox(height: 20),
        AppButton(
          text: _busy ? 'Oluşturuluyor...' : 'AI ile Oluştur',
  ...
  Widget _inputCard() {
    return AppCard(
      child: Column(
        children: [
          _numberField('Boy (cm)', _heightCtrl),
          const SizedBox(height: 12),
          _numberField('Kilo (kg)', _weightCtrl),
        ],
      ),
    );
  }
